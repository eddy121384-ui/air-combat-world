...
One buffer, float32 POSITION/NORMAL + uint32 indices, N meshes each with one
primitive, one white/grey PBR-ish material (baseColorFactor only — v0 massing).
"""
...
def write_glb(path, meshes: list[dict], material_color=(0.82, 0.83, 0.85, 1.0)) -> None:
    ...
    for m in meshes:
        ...
        p_off, n_off, i_off = push(pos_b), push(nrm_b), push(idx_b)
        ...
        gl_meshes.append({"name": m["name"], "primitives": [{
            "attributes": {"POSITION": p_ai, "NORMAL": n_ai},
