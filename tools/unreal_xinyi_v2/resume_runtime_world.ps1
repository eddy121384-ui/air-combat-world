...
if (-not (Test-Path $UnrealCmd)) {
    throw "UnrealEditor-Cmd not found: $UnrealCmd"
}
...
if (-not $InputRoot) {
    $candidates = @(
        ...
        Where-Object { $_.Name -match "^run-[0-9]+$" } |
    ...
    elseif ($candidates.Count -gt 0) {
        ...
        $gh = Get-Command gh -ErrorAction SilentlyContinue
        if (-not $gh) {
            throw "No local XinyiV2 inputs and GitHub CLI is unavailable."
        }
...
if (-not (Test-Path $Contract)) {
    throw "contract not found: $Contract"
}
...
if (-not (Test-Path $RuntimeImportReport)) {
    throw "Runtime import receipt was not written: $RuntimeImportReport"
}
...
if (-not (Test-Path $RuntimeWorldReport)) {
    throw "Runtime world receipt was not written: $RuntimeWorldReport"
}
...
if (-not (Test-Path $RuntimeWorldReopenReport)) {
    throw "Runtime world reopen receipt was not written: $RuntimeWorldReopenReport"
}
